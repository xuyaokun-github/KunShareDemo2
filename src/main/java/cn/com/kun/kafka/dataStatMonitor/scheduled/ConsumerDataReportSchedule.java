...
@EnableScheduling
@Component
public class ConsumerDataReportSchedule {
    ...
    protected void scheduled(){
        ...
        if (!kafkaDataStatMonitorProperties.isConsumerReportEnabled()){
            return;
        }
        //上报主机名
        ...
        if (!CollectionUtils.isEmpty(accumulatedTopics)){
            for (String topic : accumulatedTopics) {
                //上报主机名
                dataStoreService.addConsumerHostName(hostname);
                //上报生产者采集到的数据
                dataStoreService.countConsumerData(hostname, topic, TopicDataStatCounter.getConsumerCountMap(topic));
            }
        }
